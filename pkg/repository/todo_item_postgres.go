...
func (r *TodoItemPostgres) Create(listId int, item golang_todo_app.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]golang_todo_app.TodoItem, error) {
	var items []golang_todo_app.TodoItem
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done
									FROM %s ti INNER JOIN %s li 
         							ON li.item_id=ti.id
									INNER JOIN %s ul 
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (golang_todo_app.TodoItem, error) {
	var item golang_todo_app.TodoItem
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done
									FROM %s ti INNER JOIN %s li 
         							ON li.item_id=ti.id
									INNER JOIN %s ul 
...
func (r *TodoItemPostgres) Update(userId, itemId int, input golang_todo_app.UpdateItemInput) error {
