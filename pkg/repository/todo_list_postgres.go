...
func (r *TodoListPostgres) Create(userId int, list golang_todo_app.TodoList) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoListPostgres) GetAll(userId int) ([]golang_todo_app.TodoList, error) {
	var lists []golang_todo_app.TodoList
	query := fmt.Sprintf(
...
func (r *TodoListPostgres) GetById(userId int, listId int) (golang_todo_app.TodoList, error) {
	var list golang_todo_app.TodoList
	query := fmt.Sprintf(
		`SELECT tl.id, tl.title, tl.description
		 FROM %s tl
		 INNER JOIN %s ul ON tl.id = ul.list_id
...
func (r *TodoListPostgres) Update(userId, id int, input golang_todo_app.UpdateListInput) error {
